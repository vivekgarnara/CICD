...
class TestAdvancedAuthFlow:
    ...
    @allure.description(
        "Verify the login API returns 400 for invalid username/password"
    )
    @allure.severity("Critical")
    ...
    def test_login_missing_username(self, api_client):
        payload = {"password": "emilyspass", "expiresInMins": 30}
    ...
    def test_login_missing_password(self, api_client):
        payload = {"username": "emilys", "expiresInMins": 30}
    ...
    @allure.description(
        "Verify the get current user API returns 401 when no auth token provided"
    )
    @allure.severity("Critical")
    ...
    @allure.description(
        "Verify the get current user API returns 401 for invalid auth token"
    )
    @allure.severity("Critical")
